...
type UserService struct {
	...
	menuActionRepository repository.IMenuActionRepository
	roleRepository       repository.IRoleRepository
	roleMenuRepository   repository.IRoleMenuRepository
}
...
func (a UserService) GetUserMenuTrees(ID string) (models.MenuTrees, error) {
	...
	// Get the parent menu of the authorization menu,
	// determine which parent menus are not in the previous authorization menu,
	// and store them in the parentIDs slice
	...
	// Obtain the information of the parent menu of these differences and add it to menuResult.Data
	if len(parentIDs) > 0 {
