...
// Validator HttpServer validation function
type Validator struct {
	validate *validator.Validate
}

// BinderWithValidation Implement the bind method to verify the request's struct for parameter validation
...
func NewHttpHandler(logger Logger, config Config) HttpHandler {
	...
	echo.NotFoundHandler = func(ctx echo.Context) error {
		return echo_response.Response{Code: http.StatusNotFound}.JSON(ctx)
	}

	echo.MethodNotAllowedHandler = func(ctx echo.Context) error {
		return echo_response.Response{Code: http.StatusMethodNotAllowed}.JSON(ctx)
	}
